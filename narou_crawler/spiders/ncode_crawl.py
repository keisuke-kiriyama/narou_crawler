...
class NCodeSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page_url = response.xpath(u'//div[@class="navi_all"]/a[@title="次のページ"]/@href').extract_first()
        yield scrapy.Request(response.urljoin(next_page_url), callback=self.parse)

    def parse_mypage(self, response):
        novel_list_url = response.xpath(u'//div[@class="a_line"]/a[contains(text(), "作品一覧")]/@href').extract_first()
    ...
    def parse_novel_list(self, response):
        ...
        for url in response.xpath(u'//p[@class="info"]/a[contains(text(), "小説情報")]/@href').extract():
